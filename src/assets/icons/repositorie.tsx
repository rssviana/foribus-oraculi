import React from 'react';

function SvgRepositorie() {
  return (
    <svg width="24px" height="24px" fill="none">
      <path fill="url(#repositorie_svg__a)" d="M0 0h24v24H0z" />
      <defs>
        <pattern
          id="repositorie_svg__a"
          width={1}
          height={1}
          patternContentUnits="objectBoundingBox"
        >
          <use transform="scale(.04167)" xlinkHref="#repositorie_svg__b" />
        </pattern>
        <image
          id="repositorie_svg__b"
          width={24}
          height={24}
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}

export default SvgRepositorie;
